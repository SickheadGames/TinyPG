    ...
    public partial class Parser 
    {
            ...
            switch (tok.Type)
            {
                case TokenType.CODEBLOCK:
                    ...
                    if (tok.Type != TokenType.CODEBLOCK) {
                        tree.Errors.Add(new ParseError("Unexpected token '" + tok.Text.Replace("\n", "") + "' found. Expected " + TokenType.CODEBLOCK.ToString(), 0x1001, 0, tok.StartPos, tok.StartPos, tok.Length));
                        return;
                    }
                    break;
        ...
        private void ParseSymbol(ParseNode parent)
        {
            ...
            if (tok.Type == TokenType.UNARYOPER)
            {
                ...
                node.Nodes.Add(n);
                if (tok.Type != TokenType.UNARYOPER) {
                    tree.Errors.Add(new ParseError("Unexpected token '" + tok.Text.Replace("\n", "") + "' found. Expected " + TokenType.UNARYOPER.ToString(), 0x1001, 0, tok.StartPos, tok.StartPos, tok.Length));
                    return;
                }
            }
